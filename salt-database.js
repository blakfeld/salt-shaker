...
/**
 * Function to Add whatever Minions the Server reports as
 *  active to the database.
    ...
    salt_parser.refreshAllMinions(function(error, data) {
        ...
        if (typeof(data.length) !== 'undefined') {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                minion.save(function(error) {
                    if (error) {
                        callback(true, 'Unable to Save Minion');
                    }
                });
                ...
                salt_parser.refreshMinionGrains(data[i], function(error, data) {
                    ...
                    if (name !== '') {
                        Minion.findOneAndUpdate({'name': name},
                            {$set: {'grains': data[name]}},
                            function(error) {
                                if (error) {
                                    callback(true, error);
                                }
...
Salt_Database.prototype.addMinionGrains = function(id, callback) {
    ...
    this.getMinionById(id, function(error, data) {
        ...
        salt_parser.refreshMinionGrains(minion_name, function(error, data) {
            ...
            Minion.update({'name': minion_name},
                {$set: {'grains': data[minion_name]}});
